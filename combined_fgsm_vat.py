...
from cleverhans.attacks import FastGradientMethod, VirtualAdversarialMethod
...
def mnist_tutorial(train_start=0, train_end=60000, test_start=0,
    ...
    if clean_train:
        ...
        # Init the Virtual Adversarial Method attack object and graph
        vat = VirtualAdversarialMethod(model, sess=sess)
        adv_x_2 = vat.generate(x, **vat_params)
        preds_adv_2 = model.get_probs(adv_x_2)

        # Evaluate the accuracy of the MNIST model on Virtual Adversarial Method adversarial examples
    ...
    vat2 = VirtualAdversarialMethod(model_2, sess=sess)
    adv_x_vat = vat2.generate(x, **vat_params)
    preds_2_adv_vat = model_2(adv_x_vat)
    ...
    def evaluate_2():
        ...
        # Accuracy of the adversarially trained model on Virtual Adversarial Method adversarial examples
        accuracy = model_eval(sess, x, y, preds_2_adv_vat, X_test,
                              Y_test, args=eval_params)
        print('Test accuracy on VAT adversarial examples: %0.4f' % accuracy)

    preds_2_adv = [preds_2_adv_fgsm, preds_2_adv_vat]
